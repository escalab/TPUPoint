...
def cifar10_preprocess_image(image_bytes, is_training=False, use_bfloat16=False,
      image_size=32):
  ...
  if is_training:
    ...
    image = tf.decode_raw(image_bytes, tf.uint8)
...
class CIFAR10TFExampleInput(object):
  ...
  def __init__(self,
    ...
    self.image_preprocessing_fn = cifar10_preprocess_image #resnet_preprocessing.preprocess_image
...
class CIFAR10Input(CIFAR10TFExampleInput):
  ...
  def __init__(self,
    ...
    """
    super(CIFAR10Input, self).__init__(
  ...
  def dataset_parser(self, value):
    ...
    return super(CIFAR10Input, self).dataset_parser(value)
  ...
  def make_source_dataset(self, index, num_hosts):
    ...
    dataset = tf.data.TFRecordDataset([self.data_dir], buffer_size=8*1024*1024)
    dataset = dataset.repeat()
